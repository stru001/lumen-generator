...
namespace Stru\LumenGenerator\Generators;

use Illuminate\Support\Str;
use Stru\LumenGenerator\Common\CommandData;
use Stru\LumenGenerator\Common\GeneratorFieldRelation;
use Stru\LumenGenerator\Utils\FileUtil;
use Stru\LumenGenerator\Utils\TableFieldsGenerator;
...
class ModelGenerator extends BaseGenerator
{
    ...
    public function __construct(CommandData $commandData)
    {
        ...
        if (strpos($commandData->modelName,'/') !== false){
            ...
            $this->path = $commandData->config->pathModel.$path_fileName[0].'/';
            $this->fileName = $path_fileName[1].'.php';
        } else {
    ...
    {
        $templateData = get_template('model.model', 'lumen-generator');
    ...
    private function fillTemplate($templateData)
    {
        ...
        if ($this->commandData->getOption('primary')) {
            $primary = stru_tab()."protected \$primaryKey = '".$this->commandData->getOption('primary')."';\n";
        } else {
            $primary = '';
            if ($this->commandData->getOption('fieldsFile') && $primaryKey != 'id') {
                $primary = stru_tab()."protected \$primaryKey = '".$primaryKey."';\n";
            }
        }

        $templateData = str_replace('$PRIMARY$', $primary, $templateData);

        $templateData = str_replace('$FIELDS$', implode(','.stru_nl_tab(1, 2), $fillables), $templateData);

        $templateData = str_replace('$RULES$', implode(','.stru_nl_tab(1, 2), $rules), $templateData);

        $templateData = str_replace('$CAST$', implode(','.stru_nl_tab(1, 2), $this->generateCasts()), $templateData);

        $templateData = str_replace(
            '$RELATIONS$',
            fill_template($this->commandData->dynamicVars, implode(PHP_EOL.stru_nl_tab(1, 1), $this->generateRelations())),
            $templateData
        );
    ...
    private function fillSoftDeletes($templateData)
    {
        if (!$this->commandData->getOption('softDelete')) {
            ...
            $templateData = str_replace('$SOFT_DELETE$', stru_tab()."use SoftDeletes;\n", $templateData);
            $deletedAtTimestamp = config('lumen_generator.timestamps.deleted_at', 'deleted_at');
            $templateData = str_replace(
                '$SOFT_DELETE_DATES$',
                stru_nl_tab()."protected \$dates = ['".$deletedAtTimestamp."'];\n",
                $templateData
            );
        }
    ...
    private function fillDocs($templateData)
    {
        ...
        $docsTemplate = get_template('docs.model', 'lumen-generator');
    ...
    private function fillTimestamps($templateData)
    {
        ...
        $replace = '';
        if (empty($timestamps)) {
            $replace = stru_nl_tab()."public \$timestamps = false;\n";
        }
        ...
        if ($this->commandData->getOption('fromTable') && !empty($timestamps)) {
            ...
            $replace .= stru_nl_tab()."const CREATED_AT = $created_at;";
            $replace .= stru_nl_tab()."const UPDATED_AT = $updated_at;\n";
        }
    ...
    {
        $dont_require_fields = config('lumen_generator.options.hidden_fields', [])
                + config('lumen_generator.options.excluded_fields', $this->excluded_fields);
