...
class AdminControllerGenerator extends BaseGenerator
{
    ...
    public function __construct(CommandData $commandData)
    {
        ...
        if (strpos($commandData->modelName,'/') !== false){
            ...
            $this->path = $commandData->config->pathAdminController.$path_fileName[0].'/';
            $this->fileName = $path_fileName[1].'AdminController.php';
        } else {
            $this->path = $commandData->config->pathAdminController;
            $this->fileName = $this->commandData->modelName.'AdminController.php';
        }
    ...
    public function generate()
    {
        if ($this->commandData->getOption('repositoryPattern')) {
            $templateName = 'admin_controller';
        } else {
            $templateName = 'model_admin_controller';
        }
        ...
        $templateData = get_template("admin.controller.$templateName", 'lumen-generator');
