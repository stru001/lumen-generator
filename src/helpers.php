...
use Stru\LumenGenerator\Common\GeneratorField;
...
if (!function_exists('stru_tab')) {
     ...
     * @return string
     */
    function stru_tab($spaces = 4)
    {
        return str_repeat(' ', $spaces);
    }
}

if (!function_exists('stru_tabs')) {
    /**
     * Generates tab with spaces.
     *
     * @param int $tabs
     ...
     * @return string
     */
    function stru_tabs($tabs, $spaces = 4)
    {
        return str_repeat(stru_tab($spaces), $tabs);
    }
}

if (!function_exists('stru_nl')) {
    /**
     ...
     * @return string
     */
    function stru_nl($count = 1)
    {
        return str_repeat(PHP_EOL, $count);
    }
}

if (!function_exists('stru_nls')) {
    /**
     * Generates new line char.
     *
     * @param int $count
     ...
     * @return string
     */
    function stru_nls($count, $nls = 1)
    {
        return str_repeat(stru_nl($nls), $count);
    }
}

if (!function_exists('stru_nl_tab')) {
    /**
     * Generates new line char.
     *
     * @param int $lns
     ...
     * @return string
     */
    function stru_nl_tab($lns = 1, $tabs = 1)
    {
        return stru_nls($lns).stru_tabs($tabs);
    }
}
...
if (!function_exists('get_template_file_path')) {
    ...
    function get_template_file_path($templateName, $templateType)
    {
        ...
        $templatesPath = config(
            'lumen_generator.path.templates_dir',
...
if (!function_exists('get_templates_package_path')) {
    ...
    function get_templates_package_path($templateType)
    {
        if (strpos($templateType, '/') === false) {
            $templateType = base_path('vendor/stru/').$templateType;
        }
