...
class TableFieldsGenerator
{
    ...
    public function __construct($tableName, $ignoredFields, $connection = '')
    {
        ...
        $mappings = config('lumen_generator.from_table.doctrine_mappings', []);
        ...
        $this->defaultSearchable = config('lumen_generator.options.tables_searchable_default', false);
    }
    ...
    {
        if (!config('lumen_generator.timestamps.enabled', true)) {
            return [];
        }

        $createdAtName = config('lumen_generator.timestamps.created_at', 'created_at');
        $updatedAtName = config('lumen_generator.timestamps.updated_at', 'updated_at');
        $deletedAtName = config('lumen_generator.timestamps.deleted_at', 'deleted_at');
